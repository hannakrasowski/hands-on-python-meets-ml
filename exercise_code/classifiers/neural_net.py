...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # forward pass with applying relu at first layer

        # 1. layer
        first_layer =(np.matmul(X,W1)+ (np.ones((N,1))*b1))
        first_layer[first_layer<0]=0
        # 2. layer = output
        scores = np.matmul(first_layer,W2)+ (np.ones((N,1))*b2)
        ...
        y_predict = exp_z2 / np.sum(exp_z2, axis=1, keepdims=True)
        
        loss = -((np.sum(np.log(y_predict[N_vec,y])))/N)+ 0.5*reg*(np.sum(W1*W1) + np.sum(W2*W2))
        ...
        grads['W2'] = ((np.matmul(first_layer.T,error))/ N) + (reg * W2)
        grads['b2'] = np.mean(error,axis = 0)
        
        # gradients for first layer
        derivative_after_relu = np.matmul(error,W2.T)* ((first_layer > 0) * 1)
        
        grads['W1'] = (np.matmul(X.T,derivative_after_relu)/N) + (reg * W1)
        grads['b1'] = np.mean(derivative_after_relu,axis = 0)
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            random_num_samples = np.random.choice(num_train, batch_size)
            X_batch = X[random_num_samples,:] #shape (batch_size, dim)
            y_batch = y[random_num_samples] #shape (batch_size,)
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    for lr in np.logspace(np.log10(learning_rates[0]),np.log10(learning_rates[1]),3):
        #for rg in np.linspace(regularization_strengths[0],regularization_strengths[1],3):
    ...
    #rg = 0.05
                    ...
                    current_net = TwoLayerNet(input_size, h, num_classes)
                    current_net.train(X_train, y_train, X_val, y_val, lr, 0.95, rg, ni,200,False)

                    # compute current accuracies
                    acc_train = np.mean(current_net.predict(X_train) == y_train)
                    acc_val = np.mean(current_net.predict(X_val)== y_val)

                    # store results in hyperparameter dict and check for best accuracy on val set
                    results[(lr, rg, ni, h)] = (acc_train, acc_val)

                    if acc_val > best_val:
                        best_val = acc_val
                        best_net = current_net

                    print(acc_val,lr,rg,h,ni)
                    
                    all_classifiers.append((current_net, acc_val))
    
    
    print('best validation accuracy achieved during validation: %f' % best_val)
    
    for (lr, rg, ni, h) in sorted(results):
        train_accuracy, val_accuracy = results[(lr, rg, ni, h)]
        print('lr %e reg %e ni %d h %d val accuracy: %f' % (
              lr, rg, ni, h, val_accuracy))
       
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
    return best_net, all_classifiers
